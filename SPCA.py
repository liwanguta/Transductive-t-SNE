"""
The following class is an implementations of supervised PCA (SPCA).
...
class SPCA:
    def __init__(self, n_components=2, kernel_metric='linear'):
        # n_components = dimension for embedded data
        # kernel_metric = kernel metric used to compute the kernel matrix of target variable
    ...
    def transform(self, X=np.array([]), Y=np.array([])):
        ...
        """
            Runs SPCA on the dataset X with labels Y to reduce its dimensionality to n_compoents dimensions
        """
        ...
        Ky = pairwise_kernels(Y, metric=self.kernel_metric)
        Q = (X.T).dot(H).dot(Ky).dot(H).dot(X)
...
if __name__ == "__main__":
    ...
    SPCA = SPCA(n_components, kernel_metric)
    
    # Obtain SPCA embedding
    Z_train = SPCA.transform(X_train,Y_train).real
    Z_test = SPCA.embed_test_data(X_test).real
    ...
    Y_test_approx = SPCA.obtain_test_labels_KNN(Z_train, Z_test, Y_train, n_neighbors)
    acc = SPCA.score(Y_test, Y_test_approx)
